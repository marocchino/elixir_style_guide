...
* Git을 사용하신다면, 다음 설정을 프로젝트에 추가해 윈도우 개행으로부터
  프로젝트를 보호할 수 있습니다.
...
* 연산자 주위와 쉼표, 콜론, 세미콜론의 뒤에 공백을 넣으세요.
  중괄호나 대괄호같은 매치된 쌍주변에는 공백을 넣지 않습니다.
...
* 한 개 이상의 여러 줄 `def`가 있다면 한 줄 `def`를 사용하지 마세요.
...
* 함수를 연결할 때 파이프라인 연산자(`|>`)를 사용하세요.
  ...
  # 패턴 매칭의 오른쪽 편의 여러줄 파이프라인은 반드시 새 줄에 맞춰 개행함
  sanitized_string =
    some_string
    |> String.downcase
    |> String.strip
...
* `cond` 안에서 애텀은 참인 값으로 평가되기 때문에 다 잡는
  표현식으로 사용할 수 있습니다.
